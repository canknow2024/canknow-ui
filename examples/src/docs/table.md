...
:::demo Use `row-class-name` in `c-table` to add custom classes to a certain row. Then you can style it with custom classes.
```html
<template>
  <c-table
    :data="tableData"
...
:::demo By setting the attribute `height` of `c-table`, you can fix the table header without any other codes.
```html
<template>
  <c-table
    :data="tableData"
...
:::demo Attribute `fixed` is used in `c-table-column`, it accepts a `Boolean`. If `true`, the column will be fixed at left. It also accepts two string literals: 'left' and 'right', both indicating that the column will be fixed at corresponding direction.
```html
<template>
  <c-table
    :data="tableData"
    ...
    <c-table-column
      ...
      <template slot-scope="scope">
        <c-button @click="handleClick" size="small">Detail</c-button>
        <c-button size="small">Edit</c-button>
      </template>
    </c-table-column>
  </c-table>
</template>
...
:::demo  By setting the attribute `max-height` of `c-table`, you can fix the table header. The table body scrolls only if the height of the rows exceeds the max height value.
```html
<template>
  <c-table
    :data="tableData"
...
:::demo Only need to place c-table-column inside a c-table-column, you can achieve group header.
```html
<template>
  <c-table
    :data="tableData"
    ...
    <c-table-column label="Delivery Info">
      <c-table-column
        prop="name"
        label="Name"
        width="120">
      </c-table-column>
      <c-table-column label="Address Info">
        <c-table-column
          prop="state"
          label="State"
          width="120">
...
<template>
  ...
  <div style="margin-top: 20px">
    <c-button @click="setCurrent(tableData[1])">Select second row</c-button>
    <c-button @click="setCurrent()">Clear selection</c-button>
  </div>
</template>
...
<template>
  ...
  <div style="margin-top: 20px">
    <c-button @click="toggleSelection([tableData[1], tableData[2]])">Toggle selection status of second and third rows</c-button>
    <c-button @click="toggleSelection()">Clear selection</c-button>
  </div>
</template>
...
:::demo Set attribute `filters` and `filter-method` in `c-table-column` makes this column filterable. `filters` is an array, and `filter-method` is a function deciding which rows are displayed. It has three parameters: `value`, `row` and `column`.
```html
<template>
  <c-button @click="resetDateFilter">reset date filter</c-button>
  <c-button @click="clearFilter">reset all filters</c-button>
  <c-table
    ref="filterTable"
    :data="tableData"
    style="width: 100%">
    ...
    <c-table-column
      ...
      <template slot-scope="scope">
        <c-tag
          ...
          disable-transitions>{{scope.row.tag}}</c-tag>
      </template>
    </c-table-column>
  </c-table>
</template>
...
<template>
  <c-table
    ...
    <c-table-column
      ...
      width="180">
      <template slot-scope="scope">
        <c-popover trigger="hover" placement="top">
          <p>Name: {{ scope.row.name }}</p>
          <p>Addr: {{ scope.row.address }}</p>
          <div slot="reference" class="name-wrapper">
            <c-tag size="medium">{{ scope.row.name }}</c-tag>
          </div>
        </c-popover>
      </template>
    </c-table-column>
    <c-table-column
      ...
      <template slot-scope="scope">
        <c-button
          size="smaller"
          @click="handleEdit(scope.$index, scope.row)">Edit</c-button>
        <c-button
          size="smaller"
          scene="danger"
          @click="handleDelete(scope.$index, scope.row)">Delete</c-button>
      </template>
    </c-table-column>
  </c-table>
</template>
...
:::demo Activate expandable row by adding type="expand" and scoped slot. The template for c-table-column will be rendered as the contents of the expanded row, and you can access the same attributes as when you are using `Scoped slot` in custom column templates.
```html
<template>
  <c-table
    :data="tableData"
...
:::demo To customize row indices, use `index` attribute on `c-table-column` with `type=index`. If it is assigned to a number, all indices will have an offset of that number. It also accepts a method with each index (starting from `0`) as parameter, and the returned value will be displayed as index.
...
| size | size of Table | string | medium / small / smaller | — |
...
| resizable | whether column width can be resized, works when `border` of `c-table` is `true` | boolean | — | false |

...
:::demo Use `c-time-select` label, then assign start time, end time and time step with `start`, `end` and `step`.
```html
<c-time-select
  v-model="value"
  :picker-options="{
...
:::demo Use `c-time-picker` label, and you can limit the time range by specifying `selectableRange`. By default, you can scroll the mouse wheel to pick time, alternatively you can use the control arrows when the `arrow-control` attribute is set.
...
| is-range | whether to pick a time range, only works with `<c-time-picker>` | boolean | — | false |
| arrow-control | whether to pick time using arrow buttons, only works with `<c-time-picker>` | boolean | — | false |
...
| prefix-icon | Custom prefix icon class | string | — | c-icon-time |
| clear-icon | Custom clear icon class | string | — | c-icon-circle-close |

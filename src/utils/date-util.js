...
export const getI18nSettings = () => {
  return {
    dayNamesShort: weeks.map(week => t(`core.datepicker.weeks.${ week }`)),
    dayNames: weeks.map(week => t(`core.datepicker.weeks.${ week }`)),
    monthNamesShort: months.map(month => t(`core.datepicker.months.${ month }`)),
    monthNames: months.map((month, index) => t(`core.datepicker.month${ index + 1 }`)),
    amPm: ['am', 'pm']
  };
};

...
import { addClass, removeClass, hasClass } from 'canknow-core/src/libs';
...
export default {
  ...
  data() {
    return {
      ...
      openedMenus: (this.defaultOpens && !this.collapse) ? this.defaultOpens.slice(0) : [],
      items: {},
      submenus: {}
    };
  },
  ...
  watch: {
    ...
    collapse(value) {
      ...
      this.broadcast('CSubmenu', 'toggle-collapse', value);
    }
  },
  methods: {
    updateActiveIndex(value) {
      const item = this.items[value] || this.items[this.activeIndex] || this.items[this.defaultActive];
      if (item) {

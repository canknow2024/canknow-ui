<template>
    <div class="c-rich-editor-toolbar-item" ref="toolbarItem" :class="{ active: active}">
        <c-button border type="icon" icon="list-ordered" @click="handleClick"></c-button>
    </div>
</template>
...
export default {
  ...
  methods: {
    handleClick (e) {
      ...
      if (editor.command.queryCommandState('insertOrderedList')) {
        return;
      }
      editor.command.do('insertOrderedList');

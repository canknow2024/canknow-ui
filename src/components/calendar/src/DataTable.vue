...
export default {
  props: {
    selectedDay: String, // formatted date yyyy-MM-dd
    range: {
      type: Array,
      validator(val) {
  ...
  methods: {
    ...
    cellRenderProxy({ text, type }) {
      let render = this.cCalendar.$scopedSlots.dateCell;
  ...
  computed: {
    ...
    formatedToday() {
      return this.cCalendar.formatedToday;
    },
        ...
        <tbody>
          {
            this.rows.map((row, index) => <tr
              class={{
                'c-calendar-table-row': true,
                'c-calendar-table-row-hide-border': index === 0 && this.hideHeader
              }}
              key={index}>
              {

<template>
  <transition name="zoom-in-top" @after-leave="$emit('doDestroy')">
    <div
      v-show="visible"
      class="c-time-panel c-popper"
      :class="popperClass">
      <div class="c-time-panel-content" :class="{ 'has-seconds': showSeconds }">
        <time-spinner
          ref="spinner"
          @change="handleChange"
      ...
      <div class="c-time-panel-footer">
        <button
          type="button"
          class="c-time-panel-button cancel"
          @click="handleCancel">{{ t('core.datepicker.cancel') }}</button>
        <button
          type="button"
          class="c-time-panel-button"
          :class="{confirm: !disabled}"
          @click="handleConfirm()">{{ t('core.datepicker.confirm') }}</button>
      </div>
    </div>
  </transition>
</template>
...
export default {
  ...
  watch: {
    ...
    selectableRange(value) {
      this.$refs.spinner.selectableRange = value;
    },
    defaultValue(value) {

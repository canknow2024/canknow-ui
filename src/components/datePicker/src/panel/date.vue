<template>
  <transition name="zoom-in-top" @after-enter="handleEnter" @after-leave="handleLeave">
    <div v-show="visible"
         class="c-picker-panel c-date-picker c-popper"
         :class="[{
      ...
      <div class="c-picker-panel-body-wrapper">
        ...
        <div class="c-picker-panel-body">
          <div class="c-date-picker-time-header" v-if="showTime">
            <span class="c-date-picker-editor-wrap">
              <c-input :placeholder="t('core.datepicker.selectDate')"
                       :value="visibleDate"
                       @input="value => userInputDate = value"
                       @change="handleVisibleDateChange" />
            </span>
            <span class="c-date-picker-editor-wrap" v-clickoutside="handleTimePickClose">
              <c-input ref="input"
                       @focus="timePickerVisible = true"
                       :placeholder="t('core.datepicker.selectTime')"
                       :value="visibleTime"
                       @input="value => userInputTime = value"
          ...
          <div class="c-date-picker-header"
               :class="{ 'c-date-picker-header-bordered': currentView === 'year' || currentView === 'month' }"
               v-show="currentView !== 'time'">
            <c-button type="icon"
                    @click="prevYear"
                    size="small"
                    icon="fast-backward"
                    :aria-label="t(`core.datepicker.prevYear`)"
                    class="c-picker-panel-icon-button c-date-picker-prev-button">
            </c-button>
            <c-button type="icon"
                    @click="prevMonth"
                    size="small"
            ...
            <div class="flex-adapter">
              <span @click="showYearPicker"
                    role="button"
                    class="c-date-picker-header-label">{{ yearLabel }}</span>
              <span @click="showMonthPicker"
                    v-show="currentView === 'date'"
                    role="button"
                    class="c-date-picker-header-label"
                    :class="{ active: currentView === 'month' }">{{t(`core.datepicker.month${ month + 1 }`)}}</span>
            </div>
            <c-button type="icon"
                    @click="nextMonth"
                    size="small"
                    ...
                    class="c-picker-panel-icon-button c-date-picker-next-button">
            </c-button>
            <c-button type="icon"
                    @click="nextYear"
                    size="small"
                    icon="fast-forward"
                    :aria-label="t(`core.datepicker.nextYear`)"
                    class="c-picker-panel-icon-button c-date-picker-next-button">
            </c-button>
          </div>

          <div class="c-picker-panel-content">
      ...
      <div class="c-picker-panel-footer"
           v-show="footerVisible && currentView === 'date'">
        <c-button type="text"
                  class="c-picker-panel-link-button"
                  @click="changeToNow"
                  v-show="selectionMode !== 'dates'">
          {{ t('core.datepicker.now') }}
        </c-button>
        <c-button plain
                  scene="primary"
                  class="c-picker-panel-link-button"
...
export default {
  ...
  watch: {
    ...
    timePickerVisible(value) {
      if (value) this.$nextTick(() => this.$refs.timepicker.adjustSpinners());
    },
  ...
  computed: {
    ...
    yearLabel() {
      const yearTranslation = this.t('core.datepicker.year');
      if (this.currentView === 'year') {

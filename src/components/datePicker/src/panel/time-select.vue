<template>
  <transition name="zoom-in-top" @before-enter="handleMenuEnter" @after-leave="$emit('doDestroy')">
    <div
      ref="popper"
      v-show="visible"
      :style="{ width: width + 'px' }"
      :class="popperClass"
      class="c-picker-panel time-select c-popper">
      <scrollbar noresize wrap-class="c-picker-panel-content">
...
export default {
  ...
  methods: {
    ...
    scrollToOption(selector = '.selected') {
      const menu = this.$refs.popper.querySelector('.c-picker-panel-content');

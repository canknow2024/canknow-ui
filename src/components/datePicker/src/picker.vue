<template>
  <c-input
    ...
    <icon slot="suffix"
          ...
          @click.native="handleClickIcon"
          :name="clearIcon"
          v-if="haveTrigger && showClose"></icon>
  </c-input>
  <div class="c-date-editor c-range-editor input-base"
       :class="['c-date-editor-' + type,
       pickerSize ? `c-range-editor-${ pickerSize }` : '',
       ...
       v-clickoutside="handleClose"
       v-else>
    <icon class="input range-icon" :name="triggerClass"></icon>
    <input
            autocomplete="off"
    ...
    <input
            ...
            @focus="handleFocus"
            class="range-input">
    <icon @click.native="handleClickIcon"
          :name="clearIcon"
          :class="{'show': haveTrigger && showClose}"
          class="input-icon range-close-icon"></icon>
  </div>
</template>
...
export default {
  ...
  watch: {
    ...
    defaultValue(value) {
      ...
      if (this.picker) {
        this.picker.defaultValue = value;
      }
    },
    value(value, oldVal) {
      if (!valueEquals(value, oldVal) && !this.pickerVisible && this.validateEvent) {
  ...
  computed: {
    ...
    parsedValue() {
      ...
      return Array.isArray(this.value) ? this.value.map(value => new Date(value)) : new Date(this.value);
    },
    _formItemSize() {
  ...
  methods: {
    ...
    mountPicker() {
      ...
      this.popperElement = this.picker.$el;
    ...
    emitChange(value) {
      ...
      if (!valueEquals(value, this.valueOnOpen)) {
    ...
    emitInput(value) {
      const formatted = this.formatToValue(value);

<template>
  <table
    ...
    :class="{ 'week-mode': selectionMode === 'week' }">
    <tbody>
    <tr>
      <th v-if="showWeekNumber">{{ t('core.datepicker.week') }}</th>
      <th v-for="(week, key) in WEEKS" :key="key">{{ t('core.datepicker.weeks.' + week) }}</th>
    </tr>
    <tr
      class="c-date-table-row"

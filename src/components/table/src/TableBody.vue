...
import { getStyle, hasClass, addClass, removeClass } from 'canknow-core/src/libs';
...
export default {
    ...
    return (
      <table
        ...
        <tbody>
          ...
          <c-tooltip effect={ this.table.tooltipEffect } placement="top" ref="tooltip" content={ this.tooltipContent }></c-tooltip>
        </tbody>
      </table>
    );
  },
  ...
  watch: {
    ...
    'store.states.hoverRow'(newVal, oldVal) {
      ...
      raf(() => {
        const rows = this.$el.querySelectorAll('.c-table-row');
  ...
  methods: {
    ...
    getRowClass(row, rowIndex) {
      ...
      if (this.stripe && rowIndex % 2 === 1) {
        classes.push('c-table-row-striped');
      }
    ...
    handleCellMouseEnter(event, row) {
      ...
      if (!(hasClass(cellChild, 'c-tooltip') && cellChild.childNodes.length)) {
        return;
      }
    ...
    rowRender(row, $index, treeRowData) {
      ...
      let display = true;
      if (treeRowData) {
        rowClasses.push('c-table-row-level-' + treeRowData.level);
    ...
    wrappedRowRender(row, $index) {
      ...
      if (this.hasExpandColumn && isRowExpanded(row)) {
        ...
        if (!renderExpanded) {
          log.error('renderExpanded is required.');
          return tr;
        }
        // 使用二维数组，避免修改 $index
        return [[
          ...
          <tr key={'expanded-row-' + tr.key}>
            <td colspan={ this.columnsCount } class="c-table-cell c-table-expanded-cell">

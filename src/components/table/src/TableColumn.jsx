...
export default {
  ...
  props: {
    ...
    sortOrders: {
      ...
      validator(value) {
        return value.every(order => ['ascending', 'descending', null].indexOf(order) > -1);
      }
    }
  },
  ...
  computed: {
    ...
    realHeaderAlign() {
      return this.headerAlign ? this.headerAlign : this.realAlign;
    }
  },
  ...
  methods: {
    ...
    setColumnRenders(column) {
      ...
      if (this.renderHeader) {
        console.warn('[Canknow Warn][TableColumn]Comparing to render-header, scoped-slot header is easier to use. We recommend users to use scoped-slot header.');
      }
      ...
      if (column.type === 'expand') {
        ...
        column.renderCell = (h, data) => {
          ...
          if (column.showOverflowTooltip) {
            props.class += ' c-tooltip';
            props.style = { width: (data.column.realWidth > 0 ?  data.column.realWidth : data.column.width) - 1 + 'px' };
          }
          return (<div { ...props }>
            { prefix }
            { children }

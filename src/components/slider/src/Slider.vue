<template>
  <div
      class="c-slider"
      :class="{ 'vertical': vertical, 'slider-with-input': showInput }"
      role="slider"
      :aria-valuemin="min"
      :aria-valuemax="max"
      ...
      <template v-if="markList.length > 0">
        <div>
          <div
              ...
              class="c-slider-stop c-slider-marks-stop"
              :key="key">
          </div>
        </div>
        <div class="c-slider-marks">
          <slider-marker
...
export default {
  ...
  methods: {
    ...
    setValues() {
      if (this.min > this.max) {
        log.error('[Slider]min should not be greater than max.');
        return;
      }
      ...
      if (this.range && Array.isArray(value)) {
        if (value[1] < this.min) {
          ...
          this.$emit('input', [this.min, value[1]]);
        }
        else if (value[1] > this.max) {
          this.$emit('input', [value[0], this.max]);
        }
        else {
          this.firstValue = value[0];
          this.secondValue = value[1];
          if (this.valueChanged()) {
            this.dispatch('CFormItem', 'form.change', [this.minValue, this.maxValue]);
            this.oldValue = value.slice();
          }
        }
      }
      else if (!this.range && typeof value === 'number' && !isNaN(value)) {
        if (value < this.min) {
  ...
  computed: {
    stops() {
      ...
      if (this.step === 0) {
        process.env.NODE_ENV !== 'production' && console.warn('[Canknow Warn][Slider]step should not be 0.');
        return [];
      }

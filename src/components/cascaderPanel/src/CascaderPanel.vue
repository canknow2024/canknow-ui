<template>
  <div class="c-cascader-panel" :class="{ bordered: border }" @keydown="handleKeyDown">
    <cascader-menu
      ref="menu"
...
import { scrollIntoView } from 'canknow-core/src/libs';
import {
  noop,
...
const getSibling = (el, distance) => {
  const { parentNode } = el;
  if (parentNode) {
    const siblings = parentNode.querySelectorAll('.c-cascader-node[tabindex="-1"]');
    ...
    handleKeyDown(e) {
      ...
      switch (keyCode) {
        ...
        case KeyCode.left:
          ...
          if (preMenu) {
            const expandedNode = preMenu.$el.querySelector('.c-cascader-node[aria-expanded="true"]');
        ...
        case KeyCode.right:
          ...
          if (nextMenu) {
            const firstNode = nextMenu.$el.querySelector('.c-cascader-node[tabindex="-1"]');
            focusNode(firstNode);
          }
          break;
        case KeyCode.enter:
    ...
    */
    ...
    scrollIntoView() {
      ...
      menus.forEach(menu => {
        ...
        if (menuElement) {
          const container = menuElement.querySelector('.c-scrollbar-wrap');
          const activeNode = menuElement.querySelector('.c-cascader-node.active') ||
            menuElement.querySelector('.c-cascader-node.active-path');

...
import { isEqual } from 'canknow-core/src/libs';
...
export default {
  ...
  methods: {
    ...
    renderRadio(h) {
      ...
      return (
        <c-radio
          value={ isEqual(value, checkedValue) }
          disabled={ isDisabled }
    ...
    renderCheckIcon(h) {
      return (<icon name="success" class="icon-check c-cascader-node-prefix"></icon>);
    },

    renderLoadingIcon(h) {
      return (<icon name="loading" class="icon-loading c-cascader-node-postfix load-loop"></icon>);
    },

    renderExpandIcon(h) {
      return (<icon name="right" class="icon-arrow-right c-cascader-node-postfix"></icon>);
    },
    ...
    renderContent(h) {
      ...
      return (
        <span class="c-cascader-node-label">{ vnode || node.label }</span>
      );
    }
  },
  ...
  render(h) {
    ...
    return (
      <li role="menuitem"
        ...
        class={{
          'c-cascader-node': true,
          'selectable': checkStrictly,
          'active-path': inActivePath,
          'checked-path': inCheckedPath,
          'active': isChecked,
          'disabled': disabled
        }}
        {...events}>

...
export default {
  ...
  render(h) {
    ...
    const scrollBtn = scrollable
      ? [
        <span class={['c-tabs-nav-prev', scrollable.prev ? '' : 'disabled']} on-click={scrollPrev}><icon name="arrow-left"></icon></span>,
        <span class={['c-tabs-nav-next', scrollable.next ? '' : 'disabled']} on-click={scrollNext}><icon name="arrow-right"></icon></span>
      ] : null;
    ...
    const tabs = this._l(panes, (pane, index) => {
      ...
      const btnClose = closable
        ? <c-clear-button vertical-center={false} on-click={(ev) => {
          onTabRemove(pane, ev); 
        }}>×</c-clear-button>
        : null;
      ...
      return (
        <div
          class={{
            'c-tabs-item': true,
            [`${ this.rootTabs.tabPosition }`]: true,
            'active': pane.active,
    ...
    return (
      <div class={['c-tabs-nav-wrap', scrollable ? 'scrollable' : '', `${ this.rootTabs.tabPosition }`]}>
        {scrollBtn}
        <div class={['c-tabs-nav-scroll']} ref="navScroll">
          <div
            class={['c-tabs-nav', this.rootTabs.tabPosition, stretch && ['top', 'bottom'].indexOf(this.rootTabs.tabPosition) !== -1 ? 'stretch' : '']}
            ref="nav"
            style={navStyle}
            role="tablist"
